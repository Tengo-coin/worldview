...
export function setStyleFunction(def, vectorStyleId, vectorStyles, layer, state) {
  ...
  let styleFunction;
  const layerId = lodashGet(def, `vectorStyle.${proj.id}.id`) || lodashGet(def, 'vectorStyle.id') || def.id;
  ...
  lodashEach(layerArray, (layerInLayerGroup) => {
    ...
    styleFunction = stylefunction(layerInLayerGroup, glStyle, vectorStyleId, olMap);
    // Filter Orbit Tracks
